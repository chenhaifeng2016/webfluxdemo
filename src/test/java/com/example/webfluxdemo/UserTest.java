...
public class UserTest {
    ...
    public void test() throws IOException {
        FluxExchangeResult<User> result = webTestClient.get().uri("/api/v1/users").accept(MediaType.APPLICATION_JSON)
        ...
        assert users.size() == 2;
        assert users.iterator().next().getUserName().equals("User1");
    }

    @Test
    public void test1() throws IOException {
        User user = webTestClient.get().uri("/api/v1/users/1")
    ...
    public void test2() throws IOException {
        webTestClient.get().uri("/api/v1/users/10").accept(MediaType.APPLICATION_JSON).exchange().expectStatus()
                .isNotFound();
    }
